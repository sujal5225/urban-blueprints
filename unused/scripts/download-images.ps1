...
foreach ($project in $projects) {
    ...
    foreach ($key in $project.images.Keys) {
        $photoId = $project.images[$key]
        $url = "https://images.unsplash.com/$photoId`?auto=format&fit=crop&w=1200&h=800&q=80"
        $filename = "$key.jpg"
        $destPath = Join-Path $projectDir $filename

        Write-Host "Downloading $slug/$filename from $url..."
        try {
            Invoke-WebRequest -Uri $url -OutFile $destPath -TimeoutSec 30
            Write-Host "Successfully saved $slug/$filename"
        } catch {
            Write-Host ("Error downloading " + $slug + "/" + $filename + " : " + $_)
        }
    }
}
Write-Host "All image downloads completed!"
